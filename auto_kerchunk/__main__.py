...
def cli_main_options(
    cluster_name: str = typer.Option(None, "--cluster", help="Run dask operations on this cluster"),
    ...
    if cluster_name is not None:
        ...
        with console.status(f"[green] Starting cluster", spinner="dots") as status:
            status.update(status="[green] Starting cluster: connecting to {cluster_name!r}")
            ...
            console.log(f"client: using these packages:", client.get_versions(packages=["dask_jobqueue"]))
...
def cli_multi_zarr_to_zarr(
    ...
    freq: Optional[str] = typer.Option(None, help="divide the files into groups (only for files divided by time for now). Can either be the size of the group or a frequency like '6M'."),
):
    import dask.array
